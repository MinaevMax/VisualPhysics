...
# vpython provides a clock in its namespace
...
__attrs = {'pos': 'a', 'up': 'b', 'color': 'c', 'trail_color': 'd',
           # don't use single and double quotes; available: comma, but maybe that would cause trouble
...
class standardAttributes(baseObj):
            ...
            if a in args:
                ...
                if objName == 'extrusion' and a == 'color':
                    ...
                    if isinstance(val, vector):
                        setattr(self, '_' + a, vector(val))  # '_' bypasses setters; copy of val
                    else:
                        raise AttributeError(a + ' must be a vector')
                del args[a]
    ...
    @up.setter
    def up(self, value):
        self._save_oldup = adjust_axis(self._up, value, self._axis,
                                       self._save_oldup)  # this sets self._axis and self._up
        if not self._constructing:
            self.addattr('up')
    ...
    def axis(self, value):  # sphere or ring or text or compound have no axis/size link
        if value.mag2 == 0:
            ...
            self._save_oldaxis = adjust_up(self._axis, value, self._up,
                                           self._save_oldaxis)  # this sets self._axis and self._up
        if not self._constructing:
            self.addattr('axis')
        if self._sizing:
...
def attach_arrow(o, attr, **args):  # factory function returns arrow with special attributes
    ...
    '''
    if not hasattr(o, attr): raise AttributeError(
        'Cannot attach an arrow to an object that has no "' + attr + '" attribute.')
    if not isinstance(getattr(o, attr), vector): raise AttributeError(
        'The attach_arrow attribute "' + attr + '" is not a vector.')
...
def attach_light(o, **args):  # factory function returns local_light with special attributes
    ...
    '''
    if not isinstance(o.pos, vector): raise AttributeError(
        "Cannot attach a light to an object that has no pos attribute.")
    if 'color' in args:
    ...
    if 'offset' in args:
        if not isinstance(args['offset'], vector): raise AttributeError(
            'The attach_light attribute "offset" must be a vector.')
    else:
        ...
        for v in vars:
            if isinstance(v, vector) or isinstance(v, list) or isinstance(v, tuple):
                if not isinstance(v, vector):  # legal in GlowScript: pos=[(x,y,z), (x,y,z)] and pos=[[x,y,z], [x,y,z]]
                    v = list_to_vec(v)
                if not self._constructing:
                    ret.append({'pos': vector(
                        v)})  # make a copy of the vector; it could be (and often is, e.g. in a trail) object.pos
                else:
                    ret.append(vector(v))
            ...
            if not isinstance(vars[0][0], list) and not isinstance(vars[0][0], tuple):  # plot([x,y], [x,y])
                for v in vars:
                    ...
                    if isinstance(v, list) and len(v) == 2:
                        ...
                        raise AttributeError("Plot data must be 2-dimensional, like [x,y].")
            else:  # plot([ [x,y], [x,y] ]
                for v in vars[0]:
                    if isinstance(v, tuple): v = list(v)
                    if isinstance(v, list) and len(v) == 2:
    ...
    def preresolve2(self, args):
        ...
        if 'pos' in args:
            ...
            raise AttributeError(
                "Must be plot(x,y) or plot(pos=[x,y]) or plot([x,y]) or plot([x,y], ...) or plot([ [x,y], ... ])")
...
class extrusion(standardAttributes):
    def __init__(self, **args):
        ...
        if 'color' in args:
            if isinstance(args['color'], vector):
                args['_firstcolor'] = args['color']
            else:
                args['_firstcolor'] = args['color'][0]
        ...
        for attr in ['scale', 'xscale', 'yscale', 'twist']:
            if attr in args and isinstance(args[attr], list) and len(args[attr]) != self._pathlength:
                raise AttributeError(
                    "The " + attr + " list must be the same length as the list of points on the path (" + str(
                        self._pathlength) + ").")
    ...
    def level(self, a):
        ...
        if not (type(a[0]) is list or type(a[0]) is tuple): raise AttributeError(
            "A shape must be a list of [x,y] elements.")
    ...
    def checkjointlist(self, js, name):
        ...
        for k in js:
            if k < 0 or k > self._pathlength:
                raise AttributeError(
                    str(k) + ' is not in the range of joints in the path (0-' + str(self._pathlength - 1) + ').')
...
class text(standardAttributes):
    ...
    @property
    def lower_left(self):
        return self.upper_left + norm(self.up) * (
                -self.height - self._descender - 1.5 * self.height * (self._lines - 1))
