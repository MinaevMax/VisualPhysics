...
# eliminating dependency on a Polygon library.
...
def roundc(cps, roundness=0.1, invert=False, nseg=16):
    ...
    for i in range(len(cps)):
        cp.append(vec(cps[i][0], cps[i][1], 0))  # convert [x,y] => vec(x,y,0), so can use vector functions
...
class shape_object(object):
    ...
    def rframe(self, pos=[0, 0], width=1, height=None, rotate=0, thickness=None,
        ...
        if thickness is None:
            thickness = min(height, width) * .2
        else:
            thickness = min(height, width) * thickness * 2
