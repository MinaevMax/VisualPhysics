...
class RepulsionExperiment(Process):

    def __init__(self, value):
        super(RepulsionExperiment, self).__init__()
    ...
    def on_repeat_button_press(r, self):
        ...
        self.dt = (5. * self.xstart / (mag(self.alpha.p) / self.alpha.mass) / 1e5) * 10
        ptot = self.alpha.p + self.target.p
        self.vcm = ptot / (self.alpha.mass + self.target.mass)
    ...
    def run(self):
        ...
        self.target = sphere(pos=vector(0, 0, 0), color=color.blue, make_trail=True, interval=40, retain=50)
        ...
        while True:
            ...
            self.alpha.p = self.alpha.p + F * self.dt
            self.target.p = self.target.p - F * self.dt
            self.alpha.pos = self.alpha.pos + (self.alpha.p / self.alpha.mass) * self.dt
            self.target.pos = self.target.pos + (self.target.p / self.target.mass) * self.dt
