...
class ExperimentsScreen(QtWidgets.QWidget):
    ...
    def _init_headings(self, layout: QtWidgets.QVBoxLayout):
        try:
            ...
            heading = widget.HeadingWidget(t('screens.experiments.heading'))
            heading.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
            layout.addWidget(heading)
        except KeyError:
            heading = widget.HeadingWidget(t('screens.experiments.unknownBranchSubheading').format(branch=self.branch))
            heading.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
            layout.addWidget(heading)
    ...
    def _init_buttons(self, layout: QtWidgets.QVBoxLayout):
        ...
        try:
            for i, experiment in enumerate(main.branches[self.branch]['experiments']):
                button = widget.ButtonWidget(t(f'experiments.{experiment}.name'))
        ...
        back_button = widget.ButtonWidget(t('back'))
    ...
    def _open_home_screen(self):
        self.window.setCentralWidget(screen.HomeScreen(self.window))

    def _open_experiment_screen(self, experiment: str):
        self.window.setCentralWidget(screen.ExperimentScreen(self.window, self.branch, experiment))
